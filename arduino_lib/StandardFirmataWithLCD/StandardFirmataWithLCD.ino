...
const int iT = 0b001100;
const int oT = 0b001101;
...
const int exclamacion = 0b011010;
const int interruptor = 0b000010;

// Numeros
const int numero = 0b001111;
...
void setup()
{
  ...
  lcd.createChar(1, nE_LCD);
  lcd.createChar(2, aT_LCD);
  lcd.createChar(3, eT_LCD);
  lcd.createChar(4, iT_LCD);
  lcd.createChar(5, oT_LCD);
  lcd.createChar(6, uT_LCD);
  lcd.createChar(7, uE_LCD);
  lcd.home();
...
char readCurrentSymbol() {
  int servoPos = 0;
  for (int i = 0; i < 6; i++) {
    if (servos[i].read() > 0) {
      servoPos |= (1 << (5 - i));  // Asigna 1 al bit correspondiente
    } else {
      servoPos &= ~(1 << (5 - i));  // Asigna 0 al bit correspondiente
    }
  }

  if (servoPos == numero) // Revisa si los siguiente signos son numeros
  {
    displayNumbers = true;
  } else if (servoPos == interruptor) // Revisa si los siguiente signos son letras
  {
...
char searchChar(int servoPos) {
  if (displayNumbers) {
    return searchNumber(servoPos);
  } else {
    return searchLetter(servoPos);
  }
}

char searchLetter(int servoPos) {
  if (servoPos == a) {
    return 'a';
  } else if (servoPos == b) {
    return 'b';
...
char searchNumber(int servoPos) {
  if (servoPos == numero) {
    return '#';
